...
trait PaystackCustomerTrait
{
    public function createCustomerProfile(object $user)
    {
        ...
        $apiRequest = Http::withToken(config('app.paystack_secret'))->post($url, [
            'first_name' => $user->firstName,
            'last_name' => $user->lastName,
            'phone' => $user->phoneNumber,
            'email' => $user->email,
        ])->object();
        if($apiRequest->status == 'true'):
            $customer = $apiRequest->data->customer_code;
            ...
            session()->flash('error', $apiRequest->message);
        endif;
    }
